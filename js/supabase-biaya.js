...
//  Storage bucket : penunjang-foto
//  Tabel metadata : penunjang_foto
//    kolom: id, kunjungan_id, storage_path, url, created_at
// ═══════════════════════════════════════

const _SB_STORAGE_URL = _SB_URL + '/storage/v1';
const _FOTO_BUCKET    = 'penunjang-foto';

/**
 * Upload satu file foto ke Supabase Storage.
 * Path di bucket: {kunjunganId}/{timestamp}_{filename}
 * Setelah upload, simpan URL ke tabel penunjang_foto.
 * Return: public URL foto.
 */
async function sb_uploadFotoPenunjang(kunjunganId, file) {
    if (!kunjunganId || !file) throw new Error('kunjunganId dan file wajib ada');

    // Buat path unik di dalam bucket
    const ext       = file.name.split('.').pop() || 'jpg';
    const fileName  = `${Date.now()}_${Math.random().toString(36).slice(2,6)}.${ext}`;
    const storagePath = `${kunjunganId}/${fileName}`;

    // Upload ke Supabase Storage (multipart tidak perlu; pakai raw binary)
    const uploadRes = await fetch(
        `${_SB_STORAGE_URL}/object/${_FOTO_BUCKET}/${storagePath}`,
        {
            method: 'POST',
            headers: {
                'apikey':        _SB_KEY,
    ...
    if (!uploadRes.ok) {
        ...
        throw new Error(err.message || 'Upload storage gagal ' + uploadRes.status);
    }

    // Ambil public URL
    const publicUrl = `${_SB_STORAGE_URL}/object/public/${_FOTO_BUCKET}/${storagePath}`;

    // Simpan metadata ke tabel penunjang_foto
    await _sbFetch('penunjang_foto', {
        method: 'POST',
        body: {
            kunjungan_id:  kunjunganId,
...
/**
 * Ambil semua URL foto tersimpan untuk satu kunjungan.
 * Return: array string URL.
 */
async function sb_getFotoPenunjang(kunjunganId) {
    if (!kunjunganId) return [];
    const rows = await _sbFetch(
        `penunjang_foto?kunjungan_id=eq.${kunjunganId}&select=url,storage_path&order=created_at.asc`
    );
 ...
 * 2. Hapus baris dari tabel penunjang_foto
 */
async function sb_deleteFotoPenunjang(url, kunjunganId) {
    // Cari storage_path dari tabel
    const rows = await _sbFetch(
        `penunjang_foto?url=eq.${encodeURIComponent(url)}&kunjungan_id=eq.${kunjunganId}&select=id,storage_path&limit=1`
    );
    if (!rows.length) return;
    const { id, storage_path } = rows[0];

    // Hapus dari Storage
    await fetch(`${_SB_STORAGE_URL}/object/${_FOTO_BUCKET}/${storage_path}`, {
        method: 'DELETE',
        headers: {
            'apikey':        _SB_KEY,
            'Authorization': 'Bearer ' + _SB_KEY
    ...
    // Hapus metadata
    await _sbFetch(`penunjang_foto?id=eq.${id}`, {
        method: 'DELETE', prefer: 'return=minimal'
    });
}
